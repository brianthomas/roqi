...
my $DEBUG = 1; # will print warning messages if set
...
#   print STDERR $resource->dump("\t");
   ...
   foreach my $subjectNode ($resource_node->getElementsByTagName($SUBJECT_TAGNAME)) {
       my @s = Tokenizer::tokenize(Util::find_text($subjectNode));
#       my $subject = lc Util::find_text($subjectNode);
#       $subject =~ s/\+/_plus_/g;
#       while ($subject =~ m/(.+):(.+)/) { $subject = $2."_".$1; }
...
#       push @subject, $subject;
   ...
   foreach my $ucd (@ucds) { 
      ...
      {
print STDERR "Is NOT added yet, so lets create the hasAvailableUCD node for $sub_id $ucd\n";
...
sub createResourceIndividual ($$) {
   ...
   my @ucds = @{$ucds_ref};
   my $rnode = $doc->createElement("r:Resource");
...
sub createSubjectNode($$$$) {
    ...
    if (0) {
       if (defined $subj_arr_ref) {
          # sub-class from each parent subject
          foreach my $parent_subject (@{$subj_arr_ref}) 
          {
             my $parent_id = getSubjectURI_str($parent_subject);
             my $parent_label = $parent_subject;
             my $parent_node = get_subject_elem ($doc, $parent_id, $parent_label );
             my $parent_sub_subclass_elem = addElement($doc,$subject_node,"rdfs:subClassOf");
             $parent_sub_subclass_elem->setAttribute("rdf:resource","#".$parent_id);
          }
       } else {
          # sub-class directly from subject
...
#            </vs:column>
   ...
   foreach my $column_node (&Util::find_elements ( $table_node, $VODataService_NS_URI, $COLUMN_TAGNAME)) {
       ...
       print STDOUT "<FIELD";
       if ($#name == 0) {
           print STDOUT " name=\"".&Util::find_text($name[0])."\"";
       }
       if ($#unit == 0) {
           print STDOUT " unit=\"".&Util::find_text($unit[0])."\"";
       }
       if ($#ucd == 0) {
           print STDOUT " ucd=\"".&Util::find_text($ucd[0])."\"";
       }
       print STDOUT ">\n";
       if ($#desc == 0) {
...
sub extract_siap_metadata($) {
   ...
   if ((!$interface->getQueryType() || $interface->getQueryType() eq 'GET')
	    ...
	    if (defined $@ and $@) {
              ...
              foreach my $fieldNode (&Util::find_elements($rootNode, '', 'FIELD')) {
...
sub print_postamble ($) {
   ...
   $root->addChild(
      createObjPropNode($doc,"hasAvailableUcd", $UCD_NS_URI."#UCD","#Subject")
   );

   $root->addChild(
      createObjPropNode($doc,"hasUCD", $UCD_NS_URI."#UCD",$REGISTRY_ONTO_NS_URI."#Resource")
   );

   $root->addChild(
      createObjPropNode($doc,"hasResource", $REGISTRY_ONTO_NS_URI."#Resource", "#Subject")
   );
}
...
sub new ($) {
   ...
   foreach my $child (&Util::find_child_elements($cap_node))
   {
      ...
      if ($fieldname eq $CAPABILITY_TAGNAME) {
         ...
         $self->{$fieldname} = &Util::find_text($child);
      }
   }
   return $self;
...
sub new ($) {
   ...
   $self->{$PARAM_TAGNAME} = ();
   foreach my $child (&Util::find_child_elements($interface_node))
   {
      my $fieldname = $child->localname();
      if ($fieldname eq $PARAM_TAGNAME) {
         push @{$self->{$PARAM_TAGNAME}}, &Util::find_text($child);
...
sub new ($) {
   ...
   foreach my $child (&Util::find_child_elements($node))
   {
      my $fieldname = $child->localname();
      $self->{$fieldname} = &Util::find_text($child);
   }

   $self->{'value'} = &Util::find_text($node);
